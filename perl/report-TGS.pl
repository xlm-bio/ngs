...
if( (-e "$inputdir/genome_info.txt" ) && ( -e "$inputdir/reads_match.txt") && ( -e "$inputdir/best_par.txt")){
	...
	if ( -e "$inputdir/scaffold_info.txt"){
	print HTML"\t\t\t\t<p align=\"center\"><b>TABLE:</b> Contig/Scaffold list.</p>\n
				\t\t\t\t<div class=\"wy-table-responsive\"><table class=\"docutils\" border=\"1\">\n
				\t\t\t\t<thead>\n
				\t\t\t\t\t<tr><th>Scaffold ID</th><th>Length</th><th>GC (%)</th><th>ReadDepth</td></tr>\n
				\t\t\t\t<tbody>\n";
		open SCAFFOLD,"$inputdir/scaffold_info.txt" or die $!;
		while(<SCAFFOLD>){
			chomp;
			next if(/Scaffold/);
			my @arr=split /\t/,$_;
			print HTML"\t\t\t\t\t<tr><td>$arr[0]</td><td>$arr[1]</td><td>$arr[2]</td><td>$arr[3]</td></tr>\n";
		}
		close SCAFFOLD;
		print HTML"\t\t\t\t</tbody>\n
				\t\t\t\t</table></div>\n";
...
if( (-e "$inputdir/genome_info.txt" ) && ( !-e "$inputdir/reads_match.txt") && ( !-e "inputdir/best_par.txt")){
	...
	if ( -e "$inputdir/scaffold_info.txt"){
		print HTML"\t\t\t\t<p align=\"center\"><b>TABLE:</b> Contig/Scaffold list.</p>\n
				\t\t\t\t<div class=\"wy-table-responsive\"><table class=\"docutils\" border=\"1\">\n
				\t\t\t\t<thead>\n
				\t\t\t\t\t<tr><th>Scaffold ID</th><th>Length</th><th>GC (%)</td></tr>\n
				\t\t\t\t<tbody>\n";
		open SCAFFOLD,"$inputdir/scaffold_info.txt" or die $!;
		while(<SCAFFOLD>){
			chomp;
			next if(/Scaffold/);
			my @arr=split /\t/,$_;
			print HTML"\t\t\t\t\t<tr><td>$arr[0]</td><td>$arr[1]</td><td>$arr[2]</td></tr>\n";
		}
		close SCAFFOLD;
		print HTML"\t\t\t\t</tbody>\n
				\t\t\t\t</table></div>\n";
...
if ( (-e "$inputdir/RNA.xls") || ( -e "$inputdir/16s_blast.result.xls") || ( -e "$inputdir/$sample\_RepeatMasker/$sample\.trf.txt") || (-e "$inputdir/$sample\_piler-cr/$sample\.pilercr.result") || ( -e "$inputdir/$sample\.gene.xls")){
	...
	if( -e "$inputdir/$sample\.gene.png"){
		print  HTML"\t\t\t\t<div align=\"center\"><img src=../$sample\.gene.png></img></div>\n
